...
/* Array of supported effects of the keyboard */
...
void raw_hid_receive(uint8_t *data, uint8_t length) {
    if (!is_orgb_mode) { 
        uprintf("packet sent to via\n");
        via_raw_hid_receive(data, length); 
        return;
    }else{
        //uprintf("packet sent to openrgb\n");
    }
...
void openrgb_get_led_info(uint8_t *data) {
    ...
    for (uint8_t i = 0; i < number_leds; i++) {
        ...
        // uprintf("get_led_info: \n");
        // for(int i = 0; i<4; i++){
        //     uprintf("%d %d %d %d %d %d %d\n", raw_hid_buffer[(i*7)+1], raw_hid_buffer[(i*7)+2], raw_hid_buffer[(i*7)+3], raw_hid_buffer[(i*7)+4], raw_hid_buffer[(i*7)+5], raw_hid_buffer[(i*7)+6], raw_hid_buffer[(i*7)+7]);
        // }
        // uprintf("\n");
    }
}
...
void openrgb_set_mode(uint8_t *data) {
    ...
    if (save == 1) {
        uprintf("packet received from openrgb: mode: %d, speed: %d, hsv: %d:%d:%d\n", mode, speed, h, s, v);
